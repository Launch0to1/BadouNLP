...
def build_u2i_matrix(user_item_score_data_path, item_name_data_path, write_file=False):
    ...
    for i in range(len(user_to_rating)):
        ...
        sheet.append(line)
    workbook.save("week17/user_movie_rating.xlsx")
    return user_to_rating, item_id_to_item_name
...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    ...
    for similar_item , similarity in similar_items[item_id]:
        ...
        #用户对item_h的评分 * item与item_h的相似度，作为对item打分的加权
...
if __name__ == "__main__":
    user_item_score_data_path = "week17/ml-100k/u.data"
    item_name_data_path = "week17/ml-100k/u.item"
    ...
    while True:
        user_id = int(input("输入用户id："))
        print("UserCF结果：")
        recommands = movie_recommand(user_id, similar_user, None, user_to_rating, item_to_name)
        for recommand, score in recommands:
            print("%.4f\t%s"%(score, recommand))
        print("ItemCF结果：")
        recommands = movie_recommand(user_id, None, similar_items, user_to_rating, item_to_name)
        for recommand, score in recommands:
            print("%.4f\t%s"%(score, recommand))
